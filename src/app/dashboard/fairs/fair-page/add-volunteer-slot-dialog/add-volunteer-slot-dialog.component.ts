...
@Component({
  selector: 'app-add-volunteer-slot-dialog',
  templateUrl: './add-volunteer-slot-dialog.component.html',
  styleUrls: ['./add-volunteer-slot-dialog.component.scss']
})
export class AddVolunteerSlotDialogComponent implements OnInit, OnDestroy {
  id: string;
  addAgendaItemForm: FormGroup;
  volunteerAgendaSubscription: Subscription;

  constructor(
    private formBuilder: FormBuilder,
    private fairs: FairsService,
    private snackBar: MatSnackBar,
    private dialogRef: MatDialogRef<AddVolunteerSlotDialogComponent>,
    @Inject(MAT_DIALOG_DATA) data) {
      this.id = data.id;
     }

  ngOnDestroy(): void {
    this.volunteerAgendaSubscription.unsubscribe();
  }
  ...
  addVolunteerAgendaItem() {
    ...
    if (title && timeValue) {
      ...
      this.volunteerAgendaSubscription = this.fairs.addVolunteerAgendaItem(this.id, formattedTime, title).subscribe(
        data => {
          console.log(data);
          this.close();
